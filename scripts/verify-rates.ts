...
import {recipes,products} from "../lib/materials.ts";
...
// кожна звичайна позиція має рецепт матеріалів (або явний «без матеріалів»)
const noRecipe=catalog.filter(t=>!/^(br|kp)_/.test(t.id)&&!recipes[t.id]&&!(t.materialRate>0));
if(noRecipe.length){failed++;console.log("FAIL  без матеріалів і без рецепту: "+noRecipe.map(t=>t.id).join(", "));}
else console.log("OK    матеріали є для всіх звичайних позицій ("+Object.keys(products).length+" товарів з магазинів)");
