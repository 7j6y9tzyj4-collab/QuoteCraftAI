...
export default function AcceptBox({token,accepted,acceptedName,acceptedAt}:{token:string;accepted:boolean;acceptedName:string;acceptedAt:string}){
  ...
  async function accept(){
    ...
    setBusy(true);setErr("");
    try{
      const r=await fetch("/api/quote-accept",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token,name:name.trim()})});
