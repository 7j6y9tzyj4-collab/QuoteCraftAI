...
export type QuoteSnapshot={
  ...
  lines:QuoteLine[];
  optional:QuoteLine[];
  totals:{labor:number;materials:number;finish:number;subtotal:number;discount:number;discountLabel:string;total:number;low:number;high:number};
  notes:string;
};
...
export function buildQuoteSnapshot(input:{client:string;project:string;items:CalcItem[];locationMultiplier:number;includeFinish:boolean;totals:CalcTotals;discount:number;discountLabel:string;grandTotal:number;notes:string}):QuoteSnapshot{
